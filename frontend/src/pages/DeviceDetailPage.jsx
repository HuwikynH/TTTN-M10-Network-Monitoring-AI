...
import { aiApi, alertApi, deviceApi, metricApi } from "../api/api";
import EmptyState from "../components/EmptyState";
import DeviceAiPrediction from "../components/DeviceAiPrediction";
...
import { activeAlerts, formatDateTime, formatMetric, formatRelativeTime, highestAlertLevel, isStale, newestMetric, sortMetricsAscending } from "../utils";
...
export default function DeviceDetailPage() {
  ...
  const polling = useAutoRefresh(async (signal) => {
    if (notFound) return;
    try {
      const [deviceData, metricData, alertData, predictionData] = await Promise.all([
        deviceApi.get(numericId, { signal }),
        metricApi.listByDevice({ deviceId: numericId, limit: 120, signal }),
        alertApi.list({ deviceId: numericId, limit: 100, signal }),
        aiApi.list({ deviceId: numericId, signal }),
      ]);
      ...
      setPrediction(
        predictionData
          .filter((item) => Number(item.device_id) === numericId)
          .sort((a, b) => new Date(b.predicted_at) - new Date(a.predicted_at))[0] || null,
      );
    } catch (error) {
      if (error.status === 404) {
        setDevice(null);
  ...
  const chartData = useMemo(() => sortMetricsAscending(metrics).slice(-pointCount), [metrics, pointCount]);
  const latest = useMemo(() => newestMetric(metrics), [metrics]);
  const currentAlerts = useMemo(() => activeAlerts(alerts).sort((a, b) => new Date(b.created_at) - new Date(a.created_at)), [alerts]);
  const highestLevel = useMemo(() => highestAlertLevel(currentAlerts), [currentAlerts]);
  ...
  return (
    <div className="page">
      ...
      {device && <DeviceAiPrediction deviceName={device.name} prediction={prediction} />}
      ...
      {device && (
        <>
          ...
          <section className={"device-health-summary panel" + (highestLevel ? " device-health-summary--" + highestLevel : "")}>
            <div>
              <span className="section-label">Sức khỏe hiện tại</span>
              <div className="device-health-title"><StatusBadge status={highestLevel || "online"} label={highestLevel ? undefined : "Ổn định"} /><strong>{currentAlerts.length ? `${currentAlerts.length} cảnh báo đang hoạt động` : "Không có sự cố cần xử lý"}</strong></div>
            </div>
            {currentAlerts.length ? (
              <div className="active-alert-preview">
                {currentAlerts.slice(0, 3).map((alert) => <div key={alert.id}><StatusBadge status={alert.level} /><span>{alert.message}</span><time>{formatRelativeTime(alert.created_at)}</time></div>)}
              </div>
            ) : <p>Không có cảnh báo đang hoạt động cho thiết bị này.</p>}
            <Link className="button button--secondary" to="/alerts">Mở trung tâm cảnh báo</Link>
          </section>
          ...
          <div className="chart-toolbar">
            <div><span className="section-label">Khoảng dữ liệu</span><strong>{pointCount} mẫu gần nhất</strong></div>
            <div className="segmented-control" aria-label="Chọn số điểm biểu đồ">
              {[30, 60, 120].map((value) => <button className={pointCount === value ? "is-active" : ""} key={value} type="button" aria-pressed={pointCount === value} onClick={() => setPointCount(value)}>{value} mẫu</button>)}
            </div>
          </div>
          {!metrics.length ? <EmptyState title="Thiết bị chưa có metric" description="Biểu đồ sẽ được cập nhật khi Collector gửi dữ liệu." /> : (
            <div className="charts-grid">
              <MetricChart title="Latency" description="Độ trễ phản hồi của thiết bị" data={chartData} lines={[{ dataKey: "latency_ms", name: "Latency", color: "var(--chart-blue)" }]} unit="ms" />
              <MetricChart title="Packet loss" description="Tỷ lệ gói tin thất thoát" data={chartData} lines={[{ dataKey: "packet_loss_percent", name: "Packet loss", color: "var(--chart-orange)" }]} unit="%" domain={[0, 100]} />
              <MetricChart title="CPU" description="Mức sử dụng bộ xử lý" data={chartData} lines={[{ dataKey: "cpu_percent", name: "CPU", color: "var(--chart-purple)" }]} unit="%" domain={[0, 100]} />
              <MetricChart title="RAM" description="Mức sử dụng bộ nhớ" data={chartData} lines={[{ dataKey: "memory_percent", name: "RAM", color: "var(--chart-teal)" }]} unit="%" domain={[0, 100]} />
